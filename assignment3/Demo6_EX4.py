...
while len(input_bytes) > 0:
    ...
    # w(n) = x(n) - a1 w(n-1) - a2 w(n-2) - a3 w(n-3) - a4 w(n-4)
    # y(n) = b0 w(n) + b2 w(n-2) + b4 w(n-4)
    ...
    canonical_output_record.append(output_value)
...
print('* Canonical Form Filter Finished *')
stream.stop_stream()
stream.close()
p.terminate()
...
while len(input_bytes) > 0:
    ...
    # Compute output value
    output_value = int(clip16(y0))    # Integer in allowed range
    direct_output_record.append(output_value)
...
print('* Direct Form Filter Finished *')

stream.stop_stream()
stream.close()
p.terminate()

# Verify the out put of them
if canonical_output_record == direct_output_record:
    print("The outputs produced by Canonical Form and Direct Form is the same!")